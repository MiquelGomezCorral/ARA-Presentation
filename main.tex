...
\institute[UPV]{Máster Universitario en Inteligencia Artificial,
...
\begin{frame}{Introducción y motivación}
    En la práctica clínica \hl{no siempre se dispone de todos los datos} de un paciente al momento de decidir.
  ...
  \begin{columns}[T]
    \begin{column}{0.50\textwidth}
      ...
      \begin{tabular}{lr}
        ...
        Clínicos                & KPS, IDH1, MGMT, GTR \\
        \midrule
        Radiómicos              & 144 por región (ED, ET, NC) \\
        \bottomrule
      \end{tabular}
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{block}{MRI 3D}
        \small
        \textbf{Procesadas}: Alineadas y sin cráneo \\
        \vspace{0.3em}
        \textbf{T1}: Anatomía estructural sana \\
        \textbf{T1ce}: Tumor activo \\
...
\begin{frame}{Dataset UPenn-GBM}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      ...
      \begin{block}{Clínicos}
        \small
        \begin{itemize}
          \item \textbf{KPS}: Independencia física del paciente.
          \item \textbf{IDH1}: Estado de mutación.
          \item \textbf{MGMT}: Efectividad quimioterapia.
          \item \textbf{GTR}: Éxito extirpación completa.
        \end{itemize}
        \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Dataset UPenn-GBM}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \centering
      \includegraphics[width=0.7\textwidth]{images/radiomic_segmentation.png}

      \begin{block}{Radiomicos}
        \small
        \begin{itemize}
          \item Autosegmentación de tumor en regiones ED, ET, NC:
          \item \textbf{ED}: Edema peritumoral.
          \item \textbf{ET}: Tumor activo (tejido que capta contraste).
          \item \textbf{NC}: Núcleo necrótico (tejido muerto).
        \end{itemize}
        \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Dataset Extra: BraTS 2021}
  ...
  \end{column}
  \begin{column}{0.46\textwidth}
    \begin{block}{Autosupervisado en BraTS}
      \small
...
\begin{frame}{Descripción de la tarea}
      ...
      \begin{block}{El reto central}
        \small En inferencia, \textbf{cualquier combinación de estas entradas puede estar ausente}. El modelo debe funcionar bien en todos los casos.
      \end{block}
\end{frame}
...
\begin{frame}{Trabajo relacionado}
  ...
  \textbf{Punto de partida}: tomamos esta arquitectura y la extendemos para \hl{tolerar datos faltantes} e incorporar radiómicos como nueva modalidad.
    ...
    \begin{column}{0.4\textwidth}
      \centering
      \fbox{\includegraphics[width=\textwidth]{images/gomaa_architecture.png}}
      \vspace{0.2em}
...
% ============================================================
\begin{frame}{Preprocesado — Pipeline de datos}
  \begin{columns}[T]
    \begin{column}{0.32\textwidth}
      \begin{block}{Imágenes}
        \footnotesize
        \textbf{Formato NIfTI 3D médico}
        \begin{itemize}
          \item Eliminación de duplicados
          \item Redimensión a 96\textsuperscript{3}
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{Radiómicos}
        \footnotesize
        \textbf{12 grupos: 4 mod.\ $\times$ 3 regiones (ED, ET, NC)}
        \begin{itemize}
          \item 144 \textit{features} por grupo
          \item \textbf{12 scalers} independientes (uno por grupo) usando mediana/IQR --- robusto a \textit{outliers} médicos
          \item Ajuste \textbf{solo en train} se aplica en val y test
        \end{itemize}
      \end{block}
    \end{column}
    \begin{column}{0.32\textwidth}
      \begin{block}{Tabulares}
        \footnotesize
        \textbf{14 features clínicas}
        \begin{itemize}
          \item \textbf{Continuas}: Género $\{-1,1\}$; Edad $(x/50)-1\in[-1,1]$
          \item \textbf{Categóricas} (KPS, IDH1, MGMT, GTR) $\to$ \textit{one-hot} 
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
...
\begin{frame}{Arquitectura — Propuesta inicial}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \centering
      \fbox{\includegraphics[width=\textwidth]{images/init.png}}
      \vspace{0.3em}
...
\begin{frame}{Arquitectura — Gated Fusion}
  \begin{columns}[T]
    ...
    \begin{column}{0.46\textwidth}
      \centering
      \fbox{\includegraphics[width=\textwidth]{images/final.png}}
      % \vspace{0.3em}
  ...
  \begin{columns}[T]
    \begin{column}{0.48\textwidth}
      ...
      \begin{itemize}
        ...
        \item BraTS 2021: 1251 Muestras
        \item Upenn-GBM: 482 Muestras
      \end{itemize}

      \medskip
      \textbf{Fase 2 — Entrenamiento completo}
      \begin{itemize}
    ...
    \begin{column}{0.48\textwidth}
      ...
      \begin{block}{Estrategia train vs.\ test}
        \small
        \begin{itemize}
        \item  \textbf{Dynamic dropout} en entrenamiento \\ 
        \item \textbf{Static dropout} fijo en test para garantizar reproducibilidad.
        \end{itemize}
      \end{block}
    \end{column}
  \end{columns}
  ...
  \begin{columns}[T]
    ...
    \begin{column}{0.50\textwidth}
      ...
      \begin{enumerate}
        ...
        \item Validar el modelo en datasets externos para comprobar la generalización
      \end{enumerate}
    \end{column}
  \end{columns}
